...
- `backend/src/main/java/cn/techstar/pvms/backend/module/devicealarm/repository/`
...
设备与告警模块这次没有再新起一套表，而是复用 `station-archive` 已有事实：
...
- devices 的设备总览、分组比例、运维建议和展示字段
- alarms 的汇总卡片、闭环率、处置看板和标签归一化
...
- `/api/pvms/devices/monitor`
- `/api/pvms/alarms/center`
