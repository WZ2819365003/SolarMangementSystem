...
SM4.prototype = {
	...
	encrypt_ecb: function encrypt_ecb(key, data) {
		var errorMsg = "";
		if (key == undefined || key == null || key.length % 16 != 0) {
			errorMsg = "sm4 key is error!";
		}
		if (data == undefined || data == null || data.length <= 0) {
			errorMsg = "data is error!";
		}
  	...
  	console.log(Hex.encode(rkb,0,rkb.length));
		...
		for (var i = 0; i < loop; i++) {
			...
			(0, _byteUtil.arrayCopy)(oneCipher, 0, cipher, i * blockLen, blockLen);
		}

		var lessData = new Array(data.length % blockLen);
		if (lessData.length > 0) {
			(0, _byteUtil.arrayCopy)(data, loop * blockLen, lessData, 0, data.length % blockLen);
		}
		var padding = this.pkcs7padding(lessData, 1);
		oneCipher = this.one_encrypt(rk, padding);
		(0, _byteUtil.arrayCopy)(oneCipher, 0, cipher, loop * blockLen, blockLen);

		return cipher;
	},
	decrypt_ecb: function decrypt_ecb(key, data) {
		var errorMsg = "";
		if (key == undefined || key == null || key.length % 16 != 0) {
			errorMsg = "sm4 key is error!";
		}
  	...
  	console.log(Hex.encode(rkb,0,rkb.length));
		...
		(0, _byteUtil.arrayCopy)(lastPart, 0, plain, loop * blockLen, lastPart.length);

		//解密剩下部分数据
		for (var i = 0; i < loop; i++) {
			(0, _byteUtil.arrayCopy)(data, i * blockLen, tmp, 0, blockLen);
			onePlain = this.one_encrypt(nrk, tmp);
			(0, _byteUtil.arrayCopy)(onePlain, 0, plain, i * blockLen, blockLen);
		}

		return plain;
	},
	encrypt_cbc: function encrypt_cbc(key, iv, data) {
		var errorMsg = "";
		if (key == undefined || key == null || key.length % 16 != 0) {
			errorMsg = "sm4 key is error!";
		}
	...
	decrypt_cbc: function decrypt_cbc(key, iv, data) {
		var errorMsg = "";
		if (key == undefined || key == null || key.length % 16 != 0) {
			errorMsg = "sm4 key is error!";
		}
  	...
  	console.log(Hex.encode(rkb,0,rkb.length));
		...
		(0, _byteUtil.arrayCopy)(plain, 0, realPlain, 0, plain.length - blockLen);
		(0, _byteUtil.arrayCopy)(lastPart, 0, realPlain, plain.length - blockLen, lastPart.length);
